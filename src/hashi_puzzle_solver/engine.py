...
class Trainer:
        ...
        try:
            for epoch in range(1, epochs + 1):
                ...
                if ar_mode:
                    ...
                    overshoot_penalty = self.config["training"].get(
                        "overshoot_penalty", 5.0,
                    )
                    ...
                    self.current_masking_rate = get_masking_rate(
                        epoch, self.config["training"]["masking"], epochs,
                    )
    ...
    def create_model(self) -> torch.nn.Module:
        ...
        if model_type == "gcn":
            # GCN might accept kwargs or just ignore them if we updated it
            # to accept **kwargs
            # For now, GCN only accepts what it defines. We should update GCN if we want
            # to support new features there. But we are focusing on Transformer.
            # To be safe, we can filter or just assume model accepts **kwargs
            # if we updated it.
            # I didn't update GCN/GAT/GINE
            # yet.
            # So I will pass specific args if model_type is transformer, or be careful.
            # Actually, Python classes without **kwargs will crash if unexpected args
            # are passed.
            # TransformerEdgeClassifier has **kwargs now.
            model = GCNEdgeClassifier(
                **common_kwargs,
            )  # This might crash if not updated!
        elif model_type == "gat":
        ...
        with context:
            ...
            for batch_idx, data in enumerate(tqdm(loader, desc=desc, leave=False)):
                ...
                data = apply_edge_label_masking(
                    data, masking_rate, self.device, self.config,
                )
...
def get_masking_rate(
    epoch: int, masking_config: dict[str, Any], total_epochs: int,
) -> float:
    """Calculate progressive masking rate based on epoch."""
    if not masking_config.get("enabled"):
        return 0.0
...
def apply_edge_label_masking(
    ...
    if use_unused_capacity:
        data.x = data.x.clone()
        # Reset unused capacity to 0 (assuming state where we only count
        # MASKED edges as unused)
    ...
    with context():
        ...
        for batch_idx, data in enumerate(
            ...
            for _step in range(ar_steps):
                ...
                predicted_action = get_action_from_output(
                    output[edge_idx : edge_idx + 1], head_type,
                )
                ...
                current_bridges = apply_ar_action(
                    current_bridges, applied_action, edge_idx,
                )
...
def initialize_random_bridge_state(
    final_bridges: torch.Tensor, edge_mask: torch.Tensor,
) -> torch.Tensor:
    """
