...
Usage:
    python -m src.evaluate_model --model_path <path> --config <path> [--split <train|val|test>]
"""
import argparse
...
from .train_utils import calculate_batch_perfect_puzzles, get_edge_batch_indices, evaluate_puzzle
...
def load_model(
    ...
    try:
        ...
        raise RuntimeError(
            ...
            f"Please verify that the model structure matches your config. Original error: {exc}"
        ) from exc
...
def evaluate_model_batched(
    ...
    for data in iterator:
        ...
        for class_idx in range(3):
            ...
            class_correct[class_idx] += (pred[class_mask] == data.y[class_mask]).sum().item()
    ...
    perfect_puzzle_accuracy = total_perfect / total_puzzles if total_puzzles > 0 else 0.0
...
def evaluate_model_puzzle_by_puzzle(
    ...
    for idx in iterator:
        ...
        for class_idx in range(3):
            ...
            class_correct[class_idx] += (predictions[class_mask] == targets[class_mask]).sum().item()
            class_total[class_idx] += class_mask.sum().item()
...
def main():
    ...
    parser.add_argument(
        "--mode", type=str, default="puzzle", choices=["puzzle", "batch"],
        help="Evaluation mode: 'puzzle' for puzzle-by-puzzle (cleaner), 'batch' for batched (faster)"
    )
    ...
    # Set device
    device = get_device(args.device if args.device != "auto" else train_config.get('device', 'auto'))
    ...
    dataset = HashiDataset(
        root=root_path, split=args.split, size=size_filter, difficulty=difficulty_filter,
        limit=limit, use_degree=use_degree, use_meta_node=use_meta_node,
        use_row_col_meta=use_row_col_meta, use_distance=use_distance,
    ...
    print(f"Edge Accuracy:           {results['edge_accuracy']:.4f} ({results['correct_edges']}/{results['total_edges']})")
    print(f"Perfect Puzzle Accuracy: {results['perfect_puzzle_accuracy']:.4f} ({results['perfect_puzzles']}/{results['total_puzzles']})")
    print("-"*60)
    print("Per-Class Accuracy:")
    for i in range(3):
        print(f"  Class {i}: {results['per_class_accuracy'][i]:.4f} ({results['class_total'][i]} edges)")
    print("="*60)
