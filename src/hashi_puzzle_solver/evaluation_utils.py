...
def check_capacity_constraints(
    puzzle_data: dict, predicted_edges: list[tuple],
) -> dict[str, Any]:
    """
    ...
    return {
        ...
        "capacity_utilization": total_used / total_capacity
        if total_capacity > 0
        else 0,
...
def classify_puzzle_error_types(
    puzzle_data: dict, predicted_edges: list[tuple], actual_edges: list[tuple],
) -> dict[str, Any]:
    """
...
def plot_error_heatmap(
    results_df: pd.DataFrame, save_path: str | None = None
    ...
    heatmap_data = (
        results_df
        .groupby(["puzzle_size", "puzzle_difficulty"])
        .agg({
...
def plot_capacity_error_analysis(
    results_df: pd.DataFrame, save_path: str | None = None,
    ...
    for cap in sorted(results_df["node1_capacity"].unique()):
        ...
        capacity_stats.append({
            ...
            "avg_error_magnitude": cap_errors["error"].mean()
            if len(cap_errors) > 0
            else 0,
        })
    ...
    axes[0, 0].bar(
        cap_df["capacity"], cap_df["error_rate"], color="lightcoral", alpha=0.7,
    )
    ...
    axes[0, 1].bar(
        cap_df["capacity"], cap_df["avg_error_magnitude"], color="orange", alpha=0.7,
    )
    ...
    axes[1, 0].bar(
        cap_df["capacity"], cap_df["total_edges"], color="lightblue", alpha=0.7,
    )
...
def analyze_error_patterns_by_position(
    results_df: pd.DataFrame, save_path: str | None = None,
) -> tuple[
    plt.Figure, tuple[plt.Axes, plt.Axes], pd.DataFrame
]:
    """Analyze how errors vary by node positions in the grid."""
    # Calculate error rates by grid position (simplified to manhattan
    ...
    # Bin by distance
    pos_df["distance_bin"] = pd.cut(
        pos_df["avg_distance_from_center"],
        bins=5,
        labels=["Center", "Near", "Middle", "Far", "Edge"],
    )
...
def create_model_comparison_summary(
    results_df: pd.DataFrame, model_name: str, additional_metrics: dict | None = None,
) -> dict[str, Any]:
    """
