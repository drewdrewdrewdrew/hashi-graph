...
from .utils.graph_utils import generate_puzzle_graph
        ...
        if match:
            ...
            with Path(file_path).open() as f:
                try:
                    ...
                    msg = f"Unable to parse puzzle file {file_path}"
...
EdgeEndpoint = tuple[int, int] | Hashable
...
def graph_to_dict(
    ...
    for idx, node in enumerate(g.nodes()):
        ...
        if isinstance(node, tuple) and len(node) == 2:
            ...
            msg = f"Node {node} must be a (x, y) tuple or have 'x' and 'y' attributes"
...
def create_dataset(
    ...
    if next_puzzle_id > 0:
        ...
        print(
            f"  Existing splits - Train: {existing_splits['train']}, "
    ...
    for idx, (g, _hashi_puzzle) in enumerate(puzzle_results):
        # Determine split
        if idx in train_indices:
            ...
            msg = f"Could not determine split for puzzle at index {idx}."
            raise ValueError(msg)
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=[0],
        choices=[0, 1, 2],
        help="Puzzle difficulty: 0=easy, 1=medium, 2=hard. Can be a list. "
             "(default: [0])",
    )
    parser.add_argument(
        "--islands-pct",
    ...
    difficulties = (
        args.difficulty if isinstance(args.difficulty, list) else [args.difficulty]
    )
    ...
    for size in sizes:
        for difficulty in difficulties:
            print(
                f"\n--- Generating puzzles of size {size}x{size}, "
