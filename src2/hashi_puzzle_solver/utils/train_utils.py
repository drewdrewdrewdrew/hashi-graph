...
def update_node_features(
    ...
    # NOTE: For bidirectional graphs (the default in src2), summing over row (source) 
    # is enough as the backward edge will be caught when its own source is processed.
    # If we also sum over puzzle_col here, we double-count for bidirectional graphs.
    # Since DegreeLoss only sums over src_nodes, we follow that convention.
