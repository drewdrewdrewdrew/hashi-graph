...
class BridgesPuzzle:
    ...
    def __repr__(self) -> str:
        """Return a string representation of the BridgesPuzzle."""
        return (
            f"BridgesPuzzle(params={self.params}, desc_len={len(self.description)}, "
            f"solution_len={len(self.solution)})"
        )
...
def generate_bridges(
    ...
    Args:
        ...
        difficulty: Difficulty level (default: 0)
            ...
            - 1: Medium - Requires connection-level logic (prevents loops,
              deduces required bridges)
        ...
        islands_pct: Percentage of grid cells that should be islands
        (default: None, uses generator default)
        expansion: Expansion percentage for island placement
        (default: None, uses generator default)
    ...
    if params:
        ...
        if size is not None:
            ...
            dimensions = (
                f"{height}x{height}"  # Default to square if only height provided
            )
        else:
            dimensions = None  # Use generator defaults

        if dimensions:
            # Build parameter string: {dim}[i{islands_pct}][e{expansion}]
            # m{max_bridges}d{difficulty}
    ...
    try:
        ...
        msg = f"bridges_gen failed with exit code {e.returncode}.\nstderr: {e.stderr}"
...
def generate_bridges_dict(
    ...
    Same as generate_bridges() but returns dictionaries instead of
    BridgesPuzzle objects.
...
def build_params_string(
    dimensions: str, difficulty: int, _allow_loops: bool = False,
) -> str:
    """
    ...
    return f"{dimensions}m2d{difficulty}"
...
Examples:
  ...
  %(prog)s 7 0 --count 5    # Generate 5 easy 7x7 puzzles
    ...
    try:
        ...
        msg = f"Failed to generate puzzles via bridges_gen: {exc}"
