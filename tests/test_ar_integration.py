...
from hashi_puzzle_solver.train_utils import update_node_features, select_ar_action


def test_ar_model_instantiation():
    ...
    # Features: [capacity, structural_degree, unused_capacity, conflict_status]
    x = torch.tensor([
    ...
    ], dtype=torch.float)  # 10 nodes, 4 features
...
def test_ar_loss_computation():
    ...
    output_cond = torch.tensor([[0.9, 0.1], [0.3, 0.6], [0.8, 0.2]])  # Conditional predictions
    ...
    loss_reg = compute_ar_loss(
        output_reg, targets, current_bridges, edge_mask,
        head_type='regression', overshoot_penalty=2.0
    )

    # Test conditional loss
    loss_cond = compute_ar_loss(
        output_cond, targets, current_bridges, edge_mask,
        head_type='conditional'
    )
...
def test_ar_action_selection():
    ...
    edge_idx, confidence = select_ar_action(output_reg, current_bridges, edge_mask, 'regression')
    ...
    edge_idx, confidence = select_ar_action(output_cond, current_bridges, edge_mask, 'conditional')
