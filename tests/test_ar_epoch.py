...
from hashi_puzzle_solver.engine import run_ar_epoch, initialize_random_bridge_state
from hashi_puzzle_solver.train_utils import update_node_features, select_ar_action, apply_ar_action
...
def test_initialize_random_bridge_state(simple_puzzle_data):
    ...
    for _ in range(10):
        ...
        assert torch.all((current_bridges == 0) | (current_bridges == 1) | (current_bridges == 2))

        # Check that meta edges (non-masked) are 0
        assert current_bridges[~edge_mask].sum() == 0


def test_update_node_features(simple_puzzle_data, mock_model_config):
    ...
    unused_capacity_idx = 2  # unused_capacity is at index 2 (after capacity and structural_degree)
    assert torch.allclose(updated_x[:, unused_capacity_idx], expected_unused_capacity)


def test_apply_ar_action():
...
def test_select_ar_action_regression():
    ...
    edge_idx, confidence = select_ar_action(output, current_bridges, edge_mask, 'regression')
...
def test_select_ar_action_conditional():
    ...
    edge_idx, confidence = select_ar_action(output, current_bridges, edge_mask, 'conditional')

    # Should select edge 1 with action +2 (highest confidence)
    assert edge_idx == 1


def test_run_ar_epoch_basic(simple_puzzle_data, mock_model_config):
...
def test_run_ar_epoch_with_random_start(simple_puzzle_data, mock_model_config):
