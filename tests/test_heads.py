...
from hashi_puzzle_solver.models.heads import ConditionalActionHead, RegressionActionHead
...
class TestRegressionActionHead:
    ...
    def test_predict_action_bins(self) -> None:
        ...
        for val, expected in zip(test_values, expected_actions, strict=False):
            output = torch.tensor([val])
            action, _confidence = head.predict_action(output)
            assert action == expected, (
                f"Value {val} should map to action {expected}, got {action}"
            )

    def test_predict_action_confidence(self) -> None:
        ...
        for val in center_values:
            output = torch.tensor([val])
            _action, confidence = head.predict_action(output)
            assert confidence > 0.5, (
                f"Center value {val} should have high confidence, got {confidence}"
            )
        ...
        for val in boundary_values:
            ...
            assert confidence < 0.8, (
                f"Boundary value {val} should have lower confidence, got {confidence}"
            )
...
class TestConditionalActionHead:
    ...
    def test_predict_action_probabilities(self) -> None:
        ...
        assert abs(confidence - 0.48) < 0.01, (
            f"Expected confidence ~0.48, got {confidence}"
        )

    def test_predict_action_edge_cases(self) -> None:
        """Test edge cases for action prediction."""
        head = ConditionalActionHead(input_dim=64)

        # Case 1: p_ge1=0 (no bridges ever)
        test_output = torch.tensor([0.0, 0.5])
        action, _confidence = head.predict_action(test_output)
